...
{
public:
	Direct3D11Texture3D(ContextPtr parentContext, uint32_t width, uint32_t height, uint32_t depth, uint32_t numMips, FormatPtr format, bool isStreaming);
	~Direct3D11Texture3D() override;
	void initializeImpl() override;
	void initializeStreaming();
	void initializeDefault();

	void* mapImpl(uint32_t mipLevel, MapType type) override;
	void unmapImpl(uint32_t mipLevel) override;
